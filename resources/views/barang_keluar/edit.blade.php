<x-app-layout title="Edit Barang Keluar - Alur Confused" icon='<i data-lucide="package-plus" class="me-3"></i> Edit Barang Keluar'>
    <div class="container-fluid">
        ...
        <div class="card border-0 shadow-sm" style="background: var(--color-background); border-radius: 16px;">
            <div class="card-body" style="padding: 2rem;">
                <form id="barangKeluarForm" action="{{ route('barang_keluar.update', $barangKeluar->id) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control minimal-input" id="no_barang_keluar" 
                                       value="{{ $noBarangKeluar }}" readonly>
                            ...
                            <div class="form-floating mb-3">
                                <input type="date" 
                                       class="form-control minimal-input @error('tanggal_keluar') is-invalid @enderror" 
                                       id="tanggal_keluar" 
                                       name="tanggal_keluar" 
                            ...
                            <input
                                ...
                                id="reseller_id"
                                name="reseller_id"
                                value="{{ $barangKeluar->reseller_id }}"
                                >
                            ...
                            <div class="form-floating mb-3">
                                <input
                                    ...
                                    id="reseller_display"
                                    value="{{ $barangKeluar->reseller->nama_reseller }}"
                                    readonly
                                >
                                <label for="reseller_display">Reseller *</label>
                            </div>
                        </div>
                        <!-- Right Column -->
                        <div class="col-md-6">
                            ...
                            <div class="form-floating">
                                <textarea class="form-control minimal-input @error('keterangan') is-invalid @enderror" 
                                          ...
                                          name="keterangan" 
                                          style="height: 140px"
                                          placeholder="Keterangan">{{ old('keterangan', $barangKeluar->keterangan) }}</textarea>
                                ...
                                <table class="table table-striped table-hover align-middle" id="barangTable">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="font-weight: 600; width: 120px; color: var(--color-foreground);">Stok Keluar</th>
        ...
        <!-- Submit Buttons -->
            <div class="d-flex justify-content-end gap-3 mt-5">
                <a href="{{ route('barang_keluar.index') }}" class="btn btn-light minimal-btn-secondary">
                    <p class="d-flex align-items-center mb-0">
                ...
                <button
                    ...
                    <p class="d-flex align-items-center mb-0">
                        <i data-lucide="save" style="margin-right: 8px; width: 20px; height: 20px;"></i> Update Barang Keluar
                    </p>
                </button>
            </div>  
    </div>
...
<div class="modal fade" id="addBarangModal" tabindex="-1" aria-labelledby="addBarangModalLabel" aria-hidden="true">
    ...
    // Kalau reseller diganti
    const resellerField = document.getElementById('reseller_id');

    if (resellerField) {
        resellerField.addEventListener('change', function () {
            selectedBarang = [];
            updateTableDisplay();
            loadModalBarang();
        });
    ...
    // Submit
    document.getElementById('barangKeluarForm')
...
let selectedBarang = {!! json_encode(
    $barangKeluar->details->map(function($detail){
        return [
            'id' => $detail->barang->id,
            'nama' => $detail->barang->nama_barang,
            'stock' => ($detail->barang->persediaan->stock ?? 0) + $detail->qty,
            'satuan' => $detail->barang->satuan,
            'harga' => $detail->harga,
            'stock_keluar' => $detail->qty,
        ];
    })->values()
) !!};
...
function loadModalBarang() {
    const resellerId =
        document.getElementById('reseller_id').value;
    ...
    if (!resellerId) {
        ...
        filteredBarangs = semuaBarang.filter(barang =>
            barang.reseller_prices[resellerId]
        );
    }
...
function updateTableDisplay() {
    ...
    selectedBarang.forEach((item, index) => {
        ...
        <td id="jumlah-${index}">
            Rp ${(item.stock_keluar * item.harga || 0).toLocaleString('id-ID')}
        </td>
...
function updateStock(index, value) {
    selectedBarang[index].stock_keluar = value;
