<x-app-layout title="Edit Pesanan - Alur Confused" icon='<i data-lucide="package-plus" class="me-3"></i> Edit Pesanan'>
    <div class="container-fluid">
        <!-- Header -->
        <div class="row mb-4">
            <div class="col-md-6">
                <h4 class="mb-0" style="color: var(--color-foreground); font-weight: 600;">Edit Pesanan</h4>
                <p class="text-muted mb-0" style="font-size: 14px;">Edit data pesanan baru</p>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm" style="background: var(--color-background); border-radius: 16px;">
            <div class="card-body" style="padding: 2rem;">
                <form id="pesananForm" action="{{ route('pesanan.update', $pesanan->id) }}" method="POST">
                @csrf
                @method('PUT')
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-floating mb-3">
                                <input type="date" 
                                       ...
                                       id="tanggal_keluar" 
                                       name="tanggal_keluar" 
                                       value="{{ old('tanggal_keluar', \Carbon\Carbon::parse($pesanan->tanggal_keluar)->format('Y-m-d')) }}"
                                       required>
                            ...
                            <div class="form-floating mb-3">
                                <input
                                    ...
                                    class="form-control minimal-input"
                                    id="toko_display"
                                    value="{{ $pesanan->toko->nama_toko }}"
                                    readonly
                                >
                                <label for="toko_display">Toko *</label>
                            </div>
                        </div>
                        <!-- Right Column -->
                        <div class="col-md-6">
                            ...
                            <div class="form-floating">
                                <textarea class="form-control minimal-input @error('keterangan') is-invalid @enderror" 
                                          ...
                                          name="keterangan" 
                                          style="height: 140px"
                                          placeholder="Keterangan">{{ old('keterangan', $pesanan->keterangan) }}</textarea>
        ...
        <!-- Submit Buttons -->
            <div class="d-flex justify-content-end gap-3 mt-5">
                <a href="{{ route('pesanan.index') }}" class="btn btn-light minimal-btn-secondary">
                    <p class="d-flex align-items-center mb-0">
...
<div class="modal fade" id="addBarangModal" tabindex="-1" aria-labelledby="addBarangModalLabel" aria-hidden="true">
    ...
    // Kalau toko diganti
    const tokoField = document.getElementById('toko_id');

    if (tokoField) {
        tokoField.addEventListener('change', function () {
            selectedBarang = [];
            updateTableDisplay();
            loadModalBarang();
        });
...
let selectedBarang = {!! json_encode(
    $pesanan->details->map(function($detail){
        return [
            'id' => $detail->barang->id,
